...
`libopenssl`: `>= 4.1.0` (4.1.0-alpha1 is supported and pinned for source builds)
`libsrtp2`: OpenSSL 4.1-compatible build (OpenSSL source build pins 2.7.0)
`libusrsctp` : `<= 0.9.5.0`
`libwebsockets`: OpenSSL 4.1-compatible build (OpenSSL source build pins 5.0.0)

For the OpenSSL backend, libsrtp and libwebsockets must use the same OpenSSL
installation as the SDK. The source build selects the compatible versions
listed above; system dependency versions are not all checked by CMake.
mbedTLS builds retain their backend-specific dependency selections.
...
> **mbedTLS 4 note — build prerequisites:** the mbedTLS 4 line builds
> TF-PSA-Crypto from source, whose code generation needs Python 3 (3.8+)
> with the `jinja2` and `jsonschema` modules available at configure time:
>
> ```
> pip install jinja2 jsonschema
> ```
>
> Without them the mbedTLS 4 dependency build fails with
> `ModuleNotFoundError: No module named 'jinja2'` (or `'jsonschema'`).

> **mbedTLS 4 note — DTLS certificates:** the SDK clones the configured DTLS
> certificate key into each peer connection via PSA (`mbedtls_pk_import_into_psa`).
> Keys generated by the SDK (`createRtcCertificate`) set `PSA_KEY_USAGE_EXPORT`
...
### Building against OpenSSL 4.1 and using PQC

The OpenSSL backend requires **OpenSSL 4.1 or newer**. The source build is pinned to **4.1.0-alpha1**, with libwebsockets 5.0.0 and libsrtp 2.7.0 for OpenSSL 4 compatibility. The alpha is intended for testing. Applications can opt in to DTLS 1.3 with classical, hybrid, or pure PQC key exchange and authentication through `createPeerConnectionWithDtlsConfiguration()`. See [build instructions and all three application configurations](docs/DTLS_PQC.md). Existing `createPeerConnection()` calls retain DTLS 1.2 using the same OpenSSL 4.1 library.

To use an installed OpenSSL 4.1 or newer:

```shell
cmake .. -DBUILD_DEPENDENCIES=OFF -DUSE_OPENSSL=ON
```

CMake rejects older OpenSSL installations. Use `OPENSSL_ROOT_DIR` when selecting a custom installation. The mbedTLS backend remains a separate alternative.

> [!CAUTION]
> System-installed `libwebsockets` and `libsrtp` packages must be built against the same OpenSSL installation as the SDK. Rebuild them with `-DBUILD_DEPENDENCIES=ON` (or `-DBUILD_WEBSOCKETS=ON -DBUILD_SRTP=ON`) when they use a different OpenSSL version.

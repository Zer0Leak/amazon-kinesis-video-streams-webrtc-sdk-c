...
VOID onInboundPacket(UINT64 customData, PBYTE buff, UINT32 buffLen)
{
    ...
    BYTE dtlsData[SSL3_RT_MAX_PLAIN_LENGTH];
#endif
    ...
    if (buff[0] > 19 && buff[0] < 64) {
#ifdef KVS_USE_OPENSSL
        // A small ACK may release a previously buffered large application record.
        signedBuffLen = SIZEOF(dtlsData);
        CHK_STATUS(dtlsSessionProcessPacketWithBuffer(pKvsPeerConnection->pDtlsSession, buff, buffLen, dtlsData, &signedBuffLen));
        buff = dtlsData;
#else
        CHK_STATUS(dtlsSessionProcessPacket(pKvsPeerConnection->pDtlsSession, buff, &signedBuffLen));
#endif
        ...
        if (isDtlsConnected) {
            ...
            if (ATOMIC_LOAD_BOOL(&pKvsPeerConnection->sctpIsEnabled)) {
                ...
                while (signedBuffLen > 0) {
                    CHK_STATUS(putSctpPacket(pKvsPeerConnection->pSctpSession, buff, signedBuffLen));
#ifdef KVS_USE_OPENSSL
                    signedBuffLen = SIZEOF(dtlsData);
                    CHK_STATUS(dtlsSessionProcessPacketWithBuffer(pKvsPeerConnection->pDtlsSession, NULL, 0, dtlsData, &signedBuffLen));
#else
                    break;
#endif
                }
...
{
    return createPeerConnectionWithDtlsConfiguration(pConfiguration, NULL, ppPeerConnection);
}
                                                 ...
                                                 PRtcPeerConnection* ppPeerConnection)
{
    ...
    *ppPeerConnection = NULL;
    ...
    PROFILE_CALL(CHK_STATUS(createDtlsSessionWithOptions(&dtlsSessionCallbacks, pKvsPeerConnection->timerQueueHandle,
                                                         ...
                                                         &dtlsSessionOptions, &pKvsPeerConnection->pDtlsSession)),
...
STATUS setRemoteDescription(PRtcPeerConnection pPeerConnection, PRtcSessionDescriptionInit pSessionDescriptionInit)
{
    ...
    for (i = 0; i < pSessionDescription->sessionAttributesCount; i++) {
        if (STRCMP(pSessionDescription->sdpAttributes[i].attributeName, "fingerprint") == 0) {
            ...
            if (STRNCMP(pSessionDescription->sdpAttributes[i].attributeValue, DTLS_FINGERPRINT_SHA256_PREFIX, DTLS_FINGERPRINT_SHA256_PREFIX_LEN) ==
                ...
                /* The remote DTLS certificate fingerprint is immutable for the lifetime of the
                 * connection (RFC 8842): copy it once, on the first setRemoteDescription while the
                 * buffer is still empty, and never on a re-negotiation. The DTLS thread reads this
                 * buffer via strcmp while verifying; gating on dtlsHandshakeDone still races that read
                 * (the handshake can finish between the snapshot above and this copy), whereas the
                 * emptiness check writes exactly once — before DTLS exists — and skips thereafter. */
                if (IS_EMPTY_STRING(pKvsPeerConnection->remoteCertificateFingerprint)) {
        ...
        for (j = 0; j < pSessionDescription->mediaDescriptions[i].mediaAttributesCount; j++) {
            if (STRCMP(pSessionDescription->mediaDescriptions[i].sdpAttributes[j].attributeName, "ice-ufrag") == 0) {
                ...
                if (STRNCMP(pSessionDescription->mediaDescriptions[i].sdpAttributes[j].attributeValue, DTLS_FINGERPRINT_SHA256_PREFIX,
                    ...
                    // Write-once, same rationale as the session-level copy above.
                    if (IS_EMPTY_STRING(pKvsPeerConnection->remoteCertificateFingerprint)) {
                        STRNCPY(pKvsPeerConnection->remoteCertificateFingerprint,
    ...
    if (!pKvsPeerConnection->isOffer) {
        // Stores payload types for codecs that aren't recognized.
        PHashTable pUnknownCodecPayloadTypesTable = NULL;
        // Stores rtpmap information for unknown codecs.
        PHashTable pUnknownCodecRtpmapTable = NULL;
        // Bucket count is the number of m-lines in the session description, or MIN_HASH_BUCKET_COUNT, whichever is larger.
        ...
        CHK_LOG_ERR(hashTableFree(pUnknownCodecPayloadTypesTable));
        CHK_LOG_ERR(hashTableFree(pUnknownCodecRtpmapTable));
